...
def traverse(tree):
    cost = 0
    node_length = tree.val

    cost_left, length_left, cost_right, length_right = (0, 0, 0, 0)

    if tree.left is not None:
        cost_left, length_left = traverse(tree.left)

        node_length += length_left
        cost += cost_left + node_length

    if tree.right is not None:
        cost_right, length_right = traverse(tree.right)

        node_length += length_right
        cost += cost_right + node_length

    print(tree.val, cost_left, length_left, cost_right, length_right)

    return (cost, node_length)


print(traverse(ST.head))
